  ...
  if (sym->onStack || sym->iaccess)
    {
       ...
       */
      if (!IS_GB_I80 && (_G.omitFramePtr || sym->stack < INT8MIN || sym->stack > (int) (INT8MAX - getSize (sym->type))))
        {
...
fetchPairLong (PAIR_ID pairId, asmop *aop, const iCode *ic, int offset)
{
  ...
  if (isLitWord (aop))
  ...
  else
    {
      ...
      else if (!IS_GB_I80 && (aop->type == AOP_IY || aop->type == AOP_HL) && !(pairId == PAIR_IY && aop->size < 2))
        {
      ...
      else if (pairId == PAIR_HL && (IS_GB_I80 || (IY_RESERVED && (aop->type == AOP_HL || aop->type == AOP_EXSTK))) && requiresHL (aop))
        {
...
setupPairFromSP (PAIR_ID id, int offset)
{
  ...
  if (id == PAIR_DE && !IS_GB_I80) // TODO: Could hl be in use for gbz80, so it needs to be saved and restored?
    {
      emit2 ("ex de, hl");
      regalloc_dry_run_cost++;
    }
...
static void pointPairToAop (PAIR_ID pairId, const asmop *aop, int offset)
{
  switch (aop->type)
    {
    case AOP_EXSTK:
      wassertl (!IS_GB_I80, "The GBZ80/I80 doesn't have an extended stack");
...
commitPair (asmop *aop, PAIR_ID id, const iCode *ic, bool dont_destroy)
{
  ...
  /* PENDING: Verify this. */
    ...
    {
      /* Special cases */
      if ((aop->type == AOP_IY || aop->type == AOP_HL) && !IS_GB_I80 && aop->size == 2)
        {
          if (!regalloc_dry_run)
            {
...
adjustStack (int n, bool af_free, bool bc_free, bool hl_free, bool iy_free)
{
  ...
  else if (abs(n) > ((IS_RAB || IS_GB_I80) ? 127 * 4 - 1 : (optimize.codeSize ? 8 : 5)) && hl_free)
    {
      spillCached ();
      emit2 ("ld hl, #%d", n);
      emit2 ("add hl, sp");
  ...
  else if (!IS_GB_I80 && abs(n) > ((IS_RAB || IS_GB_I80) ? 127 * 4 - 1 : 8) && iy_free)
    {
      spillCached ();
      emit2 ("ld iy, #%d", n);
      emit2 ("add iy, sp");
  ...
  else if (abs(n) > ((IS_RAB || IS_GB_I80) ? 127 * 4 - 1 : 8) && bc_free)
    {
      emit2 ("ld c, l");
      emit2 ("ld b, h");
      emit2 ("ld hl, #%d", n);
  ...
  if (IS_FLOAT (type))
    {
      ...
      emit2 (IS_I80 ? "and a, #0x7f" : "res 7, a");   //clear sign bit
      regalloc_dry_run_cost += 2;
      skipbyte = size - 1;
    }
  while (size--)
...
static void genSend (const iCode *ic)
{
  ...
  wassertl (!IS_GB_I80, "Register parameters are not supported in gbz80 port");
  ...
  if (ispcall)
    {
      ...
      if (isLitWord (AOP (IC_LEFT (ic))))
        {
          emit2 ("call %s", aopGetLitWordLong (AOP (IC_LEFT (ic)), -1, FALSE));
          regalloc_dry_run_cost += 3;
        }
...
genEndFunction (iCode * ic)
{
  ...
  if (!IS_GB_I80 && !_G.omitFramePtr && sym->stack > (optimize.codeSize ? 2 : 1))
    {
      emit2 ("ld sp, ix");
  ...
  if (IC_TRUE (ic))
    {
      ...
      if (!strcmp (jval, "a"))
      ...
      else
        {
          ...
          if (IS_I80) {
            wassertl (!strcmp(jval, "7"), "Only works on sign bit");
            emit2 ("or a, a");
            inst = "M";
          } else {
            emit2 ("bit %s, a", jval);
...
genCmp (operand * left, operand * right, operand * result, iCode * ifx, int sign, const iCode * ic)
{
  ...
  if (AOP_TYPE (left) == AOP_CRY && AOP_TYPE (right) == AOP_CRY)
  ...
  else
    {
      ...
      if (IS_GB_I80 && sign && AOP_TYPE (right) != AOP_LIT)
        {
...
shiftR2Left2Result (const iCode *ic, operand *left, int offl, operand *result, int offr, int shCount, int is_signed)
{
  ...
  if (!optimize.codeSize || shCount <= 2)
    ...
    {
      bool use_b = (!IS_GB_I80 && !bitVectBitValue (ic->rSurv, B_IDX)
...
shiftL2Left2Result (operand *left, int offl, operand *result, int offr, int shCount, const iCode *ic)
{
  ...
  if (shCount == 0)
  ...
  else
    {
      ...
      if (AOP (shiftoperand)->type == AOP_REG && !IS_I80)
        {
          while (shCount--)
            {
              for (offset = 0; offset < size; offset++)
      ...
      else
        {
          ...
          // aopDump("8080 shift left 1", AOP (shiftoperand));
          while (size--)
            {
              if (IS_I80) {
                if (!offset)
                  emit3 (A_OR, ASMOP_A, ASMOP_A); /* have to clear carry */
                cheapMove(ASMOP_A, 0, AOP (shiftoperand), offset, 0);
                emit3_o (offset ? A_RLCA : A_RLA, 0, 0, 0, 0);
                cheapMove(AOP (shiftoperand), offset, ASMOP_A, 0, 0);
              } else
                emit3_o (offset ? A_RL : A_SLA, AOP (shiftoperand), offset, 0, 0);
              offset++;
            }
...
shiftL1Left2Result (operand *left, int offl, operand *result, int offr, unsigned int shCount)
{
  ...
  if (!aopInReg(result->aop, 0, A_IDX) && sameRegs (AOP (left), AOP (result)) && shCount <= 2 && offr == offl && !IS_I80) // 8080 has no general SLA
    {
      while (shCount--)
          ...
          if (offset >= byteshift)
            {
              if (aopInReg (AOP (result) , offset, A_IDX))
                ...
                if (IS_I80) {
                  // aopDump("8080 shift left 2", AOP (result));
                  if (!started)
                    emit3 (A_OR, ASMOP_A, ASMOP_A); /* have to clear carry */
                  cheapMove(ASMOP_A, 0, AOP (result), offset, 0);
                  emit3_o (started ? A_RLCA : A_RLA, 0, 0, 0, 0);
                  cheapMove(AOP (result), offset, ASMOP_A, 0, 0);
                } else
...
/*-----------------------------------------------------------------*/
/* genI80RightShift - generate code for right shifting on 8080     */
/*-----------------------------------------------------------------*/
static void
...
genPointerGet (const iCode *ic)
{
  ...
  else if (!IS_GB_I80 && AOP_TYPE (left) == AOP_IMMD && getPartPairId (AOP (result), 0) != PAIR_INVALID && getPartPairId (AOP (result), 2) != PAIR_INVALID)
   {
      PAIR_ID pair;
  ...
  if (!IS_GB_I80 && (AOP_TYPE (result) == AOP_STK || AOP_TYPE (result) == AOP_EXSTK) && size > 2 && (!rightval || AOP_TYPE (left) == AOP_IMMD))
    {
      int fp_offset, sp_offset;
  ...
  /* if this is rematerializable */
  if (!IS_GB_I80 && (getPairId (AOP (left)) == PAIR_BC || getPairId (AOP (left)) == PAIR_DE) && AOP_TYPE (result) == AOP_STK && !rightval
 ...
 if (isPair (AOP (result)) && IS_EZ80_Z80 && getPairId (AOP (left)) == PAIR_HL && !IS_BITVAR (retype))
  ...
  if (pair == PAIR_HL
           || (!IS_GB_I80 && (getPairId (AOP (left)) == PAIR_BC || getPairId (AOP (left)) == PAIR_DE)
...
genPointerSet (iCode * ic)
{
  ...
  /* Using ldir is cheapest for large memory-to-memory transfers. */
  if (!IS_GB_I80 && (AOP_TYPE (right) == AOP_STK || AOP_TYPE (right) == AOP_EXSTK) && size > 2)
    {
      int fp_offset, sp_offset;
...
genIfx (iCode *ic, iCode *popIc)
{
  ...
  if (AOP_TYPE (cond) != AOP_CRY && (!IS_BOOL (operandType (cond)) || IS_I80))
...
genAssign (const iCode * ic)
{
  ...
  if (isPair (AOP (result)))
    ...
    (!IS_GB_I80 && (AOP_TYPE (right) == AOP_STK && !_G.omitFramePtr || AOP_TYPE (right) == AOP_IY || AOP_TYPE (right) == AOP_LIT) && AOP_TYPE (result) == AOP_IY || // Use ld (nn), hl
    !IS_GB_I80 && AOP_TYPE (right) == AOP_IY && (AOP_TYPE (result) == AOP_STK && !_G.omitFramePtr || AOP_TYPE (result) == AOP_IY) || // Use ld hl, (nn)
    !IS_GB_I80 && AOP_TYPE (right) == AOP_LIT && (AOP_TYPE(result) == AOP_STK || AOP_TYPE(result) == AOP_EXSTK) && (AOP(result)->aopu.aop_stk + offset + _G.stack.offset + (AOP(result)->aopu.aop_stk > 0 ? _G.stack.param_offset : 0) + _G.stack.pushed) == 0 || // Use ex (sp), hl
    ...
    {
      if (!IS_GB_I80 &&             /* gbz80 doesn't have ldir */
